# approved-draft.md: Interceptor (page-foundry Phase 3 dogfood)

The approved copy for the Interceptor page, in page order, after copy-editing, the word scan, the pattern pass, and the humanizer pass (records in `process.md`). This is Phase 3's approval, not the freeze: per AUDIT-F1b the snapshot `copy/copy-approved.md` is written later, at the end of the copy-fit pass, once a design direction exists. The claims, the voice, and the section copy are settled here; the copy-fit pass fits them to the chosen form and then freezes. Every proof element is sourced to the brief or marked `[TK]`; no proof is fabricated; the exact install command and the quickstart output are `[TK]` until DOGFOOD-6 captures them against the live tool.

The `<title>` and meta description are copy too, and open this file as labeled entries so the voice chain covers them and Gate 6 diffs the built head against them.
...
Interceptor drives the browser you are already signed into, so your cookies and logged-in sessions stay on your machine and never move to a separate automated profile. There are real controls around that: a panic hotkey to stop a run, masking on sensitive fields, and a bundle-ID denylist that keeps automation away from Keychain, 1Password, System Settings, and banking apps. Every macOS permission the tool takes is tracked.
