...
v3.0 turns the skill from a template stamper into a contract compiler, and gives every companion invocation an evidence trail.

- **Archetypes are conversion contracts (#11).** Each archetype is now a six-block contract (goal, buyer entry states, jobs the page must do, proof requirements, CTA policy, ordering constraints) in place of a frozen numbered section list. The mapper became a contract compiler: it reads the brief's awareness state and price weight alongside the property's `foundry-log.md`. It fills the contract, then merges a second one when a page straddles two archetypes. Section order comes from the buyer's objection map inside the ordering constraints, not from a template. Added composition axes, a section-shape lexicon of three or more shapes per job, multi-skeleton generation with a MECLABS pre-read, and an anti-template check at Gate 1.
- **Nine more archetypes (#12).** `pricing-page`, `comparison-alternatives`, `docs-dev-tool-landing`, `waitlist-coming-soon`, `event-webinar`, `agency-services`, `ecommerce-product`, `changelog-launch-post`, and a shared `thank-you-post-conversion` page bring the set to sixteen archetypes plus the post-conversion page.
- **Evidence-based orchestration (#13).** The companion table became an I/O contract table (input, output, consumer, evidence) under a two-tier policy: seven core companions are required and preflight stops until they install, while enhancers degrade to reference-file fallbacks. Every run writes a `.agents/foundry/<product>/` artifact directory that each phase reads and writes. The gate report carries a per-companion evidence line. Every VOC quote now names a source, which makes the Gate 2 voice trace and the Gate 8 fabrication check mechanical against `voc.md`. Gate 1's MECLABS score is taken by a fresh context given only the rendered page and the brief. customer-research is authorized to run open-web discovery when the brief's voice-of-customer material is thin. theme-factory left the table, since frontend-design owns theming.
- **impeccable integration (#15, #16).** Pinned `pbakaus/impeccable` as an enhancer. `PRODUCT.md` is written at spec sign-off under a merge-never-clobber rule. Phase 4 persists a per-property `DESIGN.md` against the published eight-section design.md spec, and Gate 5 runs impeccable's detector over the built page with its findings treated as floors and a per-property accepted-findings file. Fifteen craft constants were absorbed into `references/design-direction.md`, each spot-checked against the pinned clone and stamped with its provenance.
- **Handoff 3.0 (#17).** The handoff package is a `00` to `06` manifest with an explicit creative-license grant beside a verbatim fabrication ban, image slots carrying dimensions and aspect ratios, a fixed acceptance-criteria template whose criterion IDs map to gate IDs (Gates 6 and 7 included), a voice-rules excerpt, and a return spec. A `design.md` projection compiles deterministically from the manifest and pins `version: alpha`, with `npx @google/design.md lint` wired in when the tool is present.
- **Catalog and gate work (#18, #21).** Asset production splits one owner per type (image, canvas-design); webapp-testing runs the browser checks at Gates 3 and 5; the motion stack is the four `remotion` skills read before use. Added site-architecture, seo-audit, ab-testing, and an opt-in post-ship distribution seam. Gate teeth landed too: a squint test in render review, focus and hover states, CLS and LCP checks with real thresholds, and a floors-not-verdicts line on every mechanical gate.
- **The comp stage.** Phase 4.5 renders the page as a comp and critiques it against the direction's own scene sentence and named references, in at most two bounded rounds, before the copy-fit pass and the build. A bounded copy-fit pass re-picks the hero finalist against the real hero form. The copy snapshot moves to the end of that pass, once form exists.
- **Migration (#19).** Phase 0 adopts the v2.x artifacts (the old `.agents/product-marketing.md` and `foundry-log`) into the new artifact directory, so existing properties carry forward without a manual port.
- **Tests.** TESTS.md gained regression entries for every v3 behavior: the contract compiler, the required-core preflight, the artifact directory, the evidence lines, the verbatim copy diff, the detect gate, the handoff manifest, and the design.md projection.
