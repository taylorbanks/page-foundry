...
Used: design-consultation, design-shotgun, design-html, plan-design-review, design-review, qa. **Add: `diagram`**, for the architecture/how-it-works diagrams for oss-project and saas-homepage sections; today those sections say "one diagram" with no producer. Rest of the command set (deploy/ops/ios/benchmark) is out of the page lane; browse stays excluded (Interceptor is the browser policy).
...
## Motion stack (remotion-dev/skills swept: 9 skills; plus one third-party candidate)
...
## Name verification (2026-07-18, 21b)

Every added name above was checked against its live listing before entering the companion table (`loop/evidence/21b/upstream-name-verification.txt`). Three drifted from this audit's text: the marketingskills PR skill lists upstream as `public-relations` (the `pr` shorthand above is this file's, not the repo's); the vercel-labs view-transitions skill lists as `vercel-react-view-transitions`; and heygen-com/hyperframes no longer lists a `css-animations` skill at all, its catalog having reorganized into `hyperframes-*` names since this sweep, which is the evaluate-before-trust flag earning its keep. The marketingskills listing also returned 47 names against the 44 this audit swept; dispositioning the new arrivals is the quarterly reconciliation's job (issue #6).
