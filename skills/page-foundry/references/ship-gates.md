...
- {one line per enhancer whose seam was in scope: analytics, ai-seo, schema, seo-audit (finding count and each disposition), site-architecture (multi-page properties: site-context.md written), competitor-profiling (when the brief named alternatives), ab-testing (repeat runs where a prior experiment's result fed this run's decisions, or a new experiment was designed at close), web-design-guidelines (live ruleset or frozen fallback), impeccable (which commands ran; at Gate 5 the detect finding count, fixes, and each accepted finding with its reason), remotion, gstack (which commands ran), and the archetype companions (pricing, competitors, aso, launch, lead-magnets, popups, signup) when their section is on the page. In scope but skipped reads "skipped (degraded: missing)" or "skipped (declined at preflight)"}
