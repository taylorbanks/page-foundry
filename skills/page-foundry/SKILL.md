...
Artifacts: reads `foundry-log.md` and any v2.x state left in old locations; writes the brief, starts `voc.md` at intake (from pointed-to sources, or from discovery when nothing was pointed to), and creates the run artifact directory on a product's first run.
...
Artifacts: reads the brief, the `voc.md` intake started, every VOC source the brief points to, and the watering-hole sources discovery chose when the brief pointed at none; extends `voc.md`, writes `persuasion-map.md`, `message-architecture.md`.
